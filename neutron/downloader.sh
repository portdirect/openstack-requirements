...
export BLOB=$(curl -H "Authorization: Bearer ${TOKEN}" https://registry.hub.docker.com/v2/${REPO}/manifests/latest | python -c "import sys, json; print json.load(sys.stdin)['fsLayers'][0]['blobSum']")

curl -L -H "Authorization: Bearer ${TOKEN}" https://registry.hub.docker.com/v2/${REPO}/blobs/${BLOB} > /tmp/wheels.tar.gz

tar xvf /tmp/wheels.tar.gz -C /tmp/ packages
